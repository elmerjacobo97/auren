...
export function CatalogSectionLink({ section }: CatalogSectionLinkProps) {
  const isAvailable = section.availability === "available";
  const content = (
    <>
      <span>
        <span className="flex items-center justify-between gap-3">
          <span className="text-xs font-bold uppercase tracking-[0.16em] text-[#52705b] dark:text-lime-300">
            {isAvailable ? "Published" : "Coming soon"}
          </span>
          <span
            aria-hidden="true"
            className={[
              "text-xl text-[#849787] motion-reduce:transition-none",
              isAvailable
        ...
        <span className="mt-4 block font-serif text-2xl font-semibold leading-tight text-[#17231d] dark:text-white">
          {isAvailable
            ? "Browse published blocks"
            : `${section.label} are on the roadmap`}
        </span>
      </span>
  ...
  if (!isAvailable) {
    return (
      <div
        className="flex h-full min-h-44 flex-col justify-between rounded-2xl border border-dashed border-[#a9bba9] bg-[#edf3e9] p-5 dark:border-slate-700 dark:bg-slate-900"
        data-catalog-availability="coming-soon"
      >
        {content}
      </div>
    );
  ...
  return (
    <Link
      ...
      data-catalog-availability="available"
      to={section.path}
    >
      {content}
    </Link>
