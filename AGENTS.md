...
- Root `check` and `test` build `@auren/schemas` first; `test` also runs the Node fixture tests for the blocks verifier (`node --test scripts/verify-blocks.test.mjs`) before Turbo.
